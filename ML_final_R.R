...
TrainModel<-function(method){
  ...
  if (method=="xgbDART"){
    model<- train(y= y,x= x, 
                  ...
                  tuneGrid = expand.grid(
                    nrounds = 1000,#c(500,1000,1500), # number of trees
                    max_depth = 3,#c(2,4,6),
                    eta = 0.3, # learning rate
                    gamma = 0, # pruning 
                    colsample_bytree = 1,
...
saveRDS(TrainedModel_rf, "Final_rf.rds")
saveRDS(TrainedModel_xgbTree, "TrainedModel_xgbTree.rds")
saveRDS(TrainedModel_xgbDART, "TrainedModel_xgbDART.rds")
